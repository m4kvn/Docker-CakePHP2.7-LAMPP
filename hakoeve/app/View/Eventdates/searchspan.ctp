...
<font size='5'>
<?php
  	$w = $week[date("w", strtotime(($span1)))];
   	  $day = date('Y年m月d日',strtotime($span1));
   	  if($day[12]==0){
    ...
    $w2 = $week2[date("w", strtotime(($span2)))];
      $day2 = date('Y年m月d日',strtotime($span2));
      if($day2[12]==0){
        $day2 = substr_replace("$day2", " ", 12,1);
      }
      if($day2[7]==0){
      $day2 = substr_replace("$day2", " ", 7,1);
      }
echo $day.="(".$w.")〜" .$day2."(".$w2.")";
?>
</font>
...
<div id="ps_box">
   ...
   <TABLE style="padding:0; float:right;margin-top:5px;">
    <tr style="padding:0;">
     <?php if($id > 1){?>
     <td class="pages"><a class="page" href="/hakoeve/eventdates/searchspan/<?php echo $span1;?>/<?php echo $span2?>?id=<?php echo $id - 1;?>">＜前へ</a></td>
     <?php }else{?>
     ...
     <?php } ?>
      ...
      if(ceil($eventNum / 20/*表示件数*/) <= 5){
       for($i = 1; $i <= ceil($eventNum / 20/*表示件数*/); $i++){
			if($id != $i){
        ?>   <td class="pages"><a class="page" href="/hakoeve/eventdates/searchspan/<?php echo $span1;?>/<?php echo $span2;?>?id=<?php echo $i;?>"><?php echo $i;?></a></td>
     <?php	}else{ ?> 
...
<?php 		}
       ...
       $p = 0;
       if($id <= 4) $p = 1;
       elseif($id > ceil($eventNum / 20/*表示件数*/ - 4)) $p = ceil($eventNum / 20/*表示件数*/ - 4);
       else $p = $id - 2;
 ...
 <?php		}
     ...
     <?php if($id < ceil($eventNum / 20)){ ?> 
      <td class="pages" style="padding-right:0;"><a class="page" href="/hakoeve/eventdates/searchspan/<?php echo $span1;?>/<?php echo $span2?>?id=<?php echo $id + 1;?>">次へ＞</a></td>
      <?php }else{ ?>
