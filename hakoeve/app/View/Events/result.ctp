...
?>
<div class= 'rect'>
<div style="float: left; height: 15px; margin-top: 5px;">
&nbsp;全<?php echo $eventNum;?>件中&nbsp;&nbsp;<?php echo count($result);?>件表示</div>
<div class="page_select_area" style="width: 400px;float: right;">
<div id="ps_box">
   <TABLE style="padding:0; float:right;margin-top:5px;">
    <tr style="padding:0;">
     <?php if($id > 1){?>
     <td class="pages"><a class="page" href="/hakoeve/events/result/keyword:<?php echo $keyword;?>?id=<?php echo $id - 1;?>">＜前へ</a></td>
     <?php }else{?>
     ...
     <?php } ?>
      ...
      if(ceil($eventNum / 20/*表示件数*/) <= 5){
       for($i = 1; $i <= ceil($eventNum / 20/*表示件数*/); $i++){
          if($i != $id){
        ?>   <td class="pages"><a class="page" href="/hakoeve/events/result/keyword:<?php echo $keyword;?>?id=<?php echo $i;?>"><?php echo $i;?></a></td>
        <?php    }else{ ?>
     ...
     <?php if($id < ceil($eventNum / 20)){ ?>
      <td class="pages" style="padding-right:0;"><a class="page" href="/hakoeve/events/result/keyword:<?php echo $keyword;?>?id=<?php echo $id + 1;?>">次へ＞</a></td>
      <?php }else{ ?>
...
//	$image=str_replace('.', '_original.',$value['Event']['thumbnail_file_name']);
	if($image[$ImageNum]['EventImage']['img_file_name'] == ""){
...
<div id="ps_box">
   ...
   <TABLE style="padding:0;float:right; margin-top: 10px;">
    <tr style="padding:0;">
     <?php if($id > 1){?>
     <td class="pages"><a class="page" href="/hakoeve/events/result/keyword:<?php echo $keyword;?>?id=<?php echo $id - 1;?>">＜前へ</a></td>
     <?php }else{?>
